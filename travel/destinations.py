...
@destbp.route('/<id>')
def show(id):
    destination = db.session.query(Destination).filter(Destination.id == id).scalar()
    form = CommentForm()
...
#         'Brazil', 'beautiful country', 'brazil.jpg', 'R$10')
#     comment = Comment('Me', "A very nice place", '13 Sep')
#     destination.set_comments(comment)
#     comment = Comment('Someone', "good place", '15 Sep')
#     destination.set_comments(comment)
#     return destination
...
def comment(id):
    ...
    if form.validate_on_submit():  
      ...
      comment = Comment(text=form.text.data, destination_id=id, user=current_user) 
